...
{
    public class NOFItemsController : Controller

    {
        ...
        public NOFItemsController(IConfiguration configuration, UserClass tokenInfo, IWebHostEnvironment webHostEnvironment)
        {
        ...
        public IActionResult ViewNOFItems()
        {

            return View();
        }

        public JsonResult GetNOFItems()
        {
            ...
            while (result_clear.Read())
            {
                ...
                item.Remarks = result_clear["remarks"].ToString();
                item.RemarksDesc = result_clear["remarksDesc"].ToString();
                items.Add(item);
            ...
            List<RemarksTable> remarksData = _userInfoConn.RemarksTable
                    .Select(item => new RemarksTable
                    {
                        code = item.code,
        ...
        public JsonResult ClearItems(NOFViewModel nOFClass)
        {
            ...
            var claims = (System.Security.Claims.ClaimsIdentity)User.Identity;
            var user = claims.Claims.ToList()[0].Value;
            for (int y = 0; y < nOFClass.NOFClasses.Count; y++)
            {
                var ordersTable = new List<OrderClass>();
                ordersTable = _userInfoConn.ordersTable.Where(e => e.sku_id == nOFClass.NOFClasses[y].sku_id && e.typeOfexception == "NOF").ToList();
                ...
                for (int x = 0; x < ordersTable.Count; x++)
                {
                    ...
                    ordersTable[x].NOFRemarks = nOFClass.NOFClasses[y].Remarks;
            ...
            return Json(new { set = nOFClass });

        }
        public JsonResult UpdateItems(NOFViewModel nOFClass)
        {


            var claims = (System.Security.Claims.ClaimsIdentity)User.Identity;
            var user = claims.Claims.ToList()[0].Value;
            if (nOFClass.NOFClasses != null)
            {
                for (int y = 0; y < nOFClass.NOFClasses.Count; y++)
                {
                    var ordersTable = new List<OrderClass>();
